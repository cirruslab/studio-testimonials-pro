...
class Controller extends BlockController {
		...
		public function view(){
			...
			} elseif ($this->display_type == 'slider') {
				$this->addFooterItem('<script type="text/javascript">
				$(function(){
				    $("#studio-testimonials-'.$this->bID.' .st-pro-slider").responsiveSlides({
				    	auto: '. ($this->auto_slide ? 'true' : 'false') .',
				    	timeout: '. $this->rotate_length .',
				    	prevText: "", 
            			nextText: "",
            			nav: true,
            			namespace: "st_slider"
		...
		protected function add_edit(){
			...
			$this->set('type_options', array('single' => t('Single'), 'single_rotate' => t('Single Rotating'), 'multiple'=>t('Multiple'), 'slider'=>t('Slider')) );
		...
		public function save($data){
			...
			$data['auto_slide'] = intval($data['auto_slide']);
